...
const Bar = () => {
  ...
  const menu = [
    { 
      id: 0,
      icon: House, // Menyimpan komponen ikon saja
      to: '#home',
      activeColor: 'mb-2 transition duration-300 ease-in-out text-orange-500', // Menggunakan text-color bukan fill
    ...
    const handleScroll = () => {
      menu.forEach(item => {
        ...
        if (section) {
          ...
          // Jika bagian atas section berada di antara 1/3 dan 2/3 viewport
  ...
  return (
    <div className="fixed bottom-2 left-0 z-50 right-0 mx-auto bg-gray-50/80 border border-gray-300 w-[300px] h-[60px] rounded-full backdrop-blur-sm">
      <div className="grid grid-cols-4 h-full items-center justify-center px-4">
        {menu.map((item) => {
          const Icon = item.icon; // Komponen ikon
          ...
          return (
            ...
            >
              <Icon 
                ...
                height={40} 
                strokeWidth={1.2} 
                fill={isActive ? "currentColor" : "none"} // Mengisi warna saat aktif
              />
            </a>
          );
        })}
